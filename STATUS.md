...
- ⬜ **D (carry) — EJECUTADA en SECO 2026-08-22** (sin credenciales, autorizada por Héctor 04:36): `scripts/carry_backtest.py` + `tests/test_carry_backtest.py` (9 tests). Funding carry (long spot + short perp 1:1) Binance USDT-M 2021-2025 → **PF agregado 3.447** (rompe el gate ≥1.5). PERO correlación vs trend **+0.43/+0.55** (NO diversifica; concentra riesgo de régimen). 2022 (bajista) cae a PF 0.51 (neto negativo). **Conclusión: edge existe vía carry, pero es riesgo de régimen concentrado, no diversificación mágica.**
- ⬜ **Funding carry en staging** (Kraken/Bybit/OKX, **requiere credenciales + OK Héctor**): pendiente. Validar funding + ejecución real con $50–100 antes de cualquier $ real.
