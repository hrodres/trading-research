...
- **Fase B completada (6 variants de entrada)**: `strategies/entry_study.py` + `scripts/entry_study.py` (orquestador `--strategy-list`) + `configs/backtest_entrystudy.json` + `results/entrystudy_B.json`. Walk-forward 2021-2025, sizing 100 USDT, sin fitting. **0 variants/par pasan el gate** (mejor por variant: EntryTrendADX PF 0.756; mejor celda: EntryVolConfirm SOL/USDT PF 1.283, n=140). Conclusión: techo trend-following 2h ~1.3; el salto a PF≥1.5 exige Fase D (diversificación). EntryMeanRev (contrarian) PF 0.066 → régimen trend-following confirmado.
- **Anexo forense (aislado, NO parte de B)**: `EntryV9Style` — estilo de entrada de v9, añadida a petición del usuario por curiosidad y luego aislada. Replica score_bull ≥2 de 3 señales de v9. Mismo exit de C, walk-forward 2021-2025. Resultado: **casi inerte** (5 trades en 5 años, PF 0.0). NO forma parte de la base de señales; evidencia en `results/entrystudy_v9style.json` (separada de `entrystudy_B.json`).
